...
const rowId = (normalized) => createHash("sha256").update(normalized).digest("hex").slice(0, 24);
const completed = new Set([
    "EXACT_CANONICAL_MATCH",
    "ALIAS_MATCH",
    "EXTERNAL_MAPPING_MATCH",
]);
let BrandImportAssistantService = class BrandImportAssistantService {
    async batches() {
    ...
    async rows(batchId, query) {
        ...
        rows = rows.filter((row) => (!query.classification ||
    ...
    async createBrand(batchId, id, input, actorId) {
        ...
        if (row.classification !== "MISSING_BRAND")
            throw new ConflictException("Csak hiányzó, konfliktusmentes márkából hozható létre új rekord.");
        const name = input.canonicalName.trim();
        if (!name)
            throw new BadRequestException("A kanonikus név kötelező.");
        if (input.createExternalMapping)
            throw new BadRequestException("Ehhez a forrásértékhez nincs igazolt stabil UNAS márkaazonosító.");
        try {
    ...
    async mapAlias(batchId, id, input, actorId) {
        await this.assertMutableBatch(batchId);
        const row = await this.currentRow(batchId, id, input.expectedUpdatedAt);
        ...
        if (!brand)
            throw new NotFoundException("A célmárka nem található.");
        if (!brand.isActive)
            throw new ConflictException("Archivált márkához nem adható új alias.");
        try {
            await prisma.$transaction(async (tx) => {
                await this.assertIdentityFree(tx, row.normalizedSourceValue, brand.id);
    ...
    async mapExternal(batchId, id, input, actorId) {
        await this.assertMutableBatch(batchId);
        const row = await this.currentRow(batchId, id, input.expectedUpdatedAt);
        if (!row.externalMapping ||
            row.externalMapping.externalId !== input.externalId)
            throw new BadRequestException("Külső megfeleltetés csak a batchből igazolt stabil UNAS azonosítóval menthető.");
        const brand = await prisma.brand.findUnique({
            where: { id: input.brandId },
        });
        if (!brand?.isActive)
    ...
    async bulkCreate(batchId, input, actorId) {
        await this.assertMutableBatch(batchId);
        if (new Set(input.rowIds).size !== input.rowIds.length)
            throw new BadRequestException("Duplikált sorazonosító.");
        const initialRows = new Map((await this.analyze(batchId)).rows.map((row) => [row.id, row]));
        const results = [];
        for (const sourceBrandId of input.rowIds) {
            ...
            try {
                const created = await prisma.$transaction(async (tx) => {
                    ...
                    const brand = await tx.brand.create({
                        data: {
                            ...
                            normalizedName: normalizeBrandName(current.proposedCanonicalName),
                            slug: normalizeBrandName(current.proposedCanonicalName).replace(/ /g, "-"),
                        },
                    });
                    await tx.domainEvent.create({
                        data: {
                ...
                const refreshed = (await this.analyze(batchId)).rows.find((row) => row.id === sourceBrandId);
                if (refreshed &&
                    ...
                    results.push({
                        ...
                        sourceName: current.sourceValue,
                        status: "CONFLICT",
                        reason: "A normalizált márkaidentitást egy párhuzamos művelet lefoglalta.",
                    });
                }
                else {
                    results.push({
                        ...
                        sourceName: current.sourceValue,
                        status: "FAILED",
                        reason: "A márka létrehozása váratlan adatbázishiba miatt sikertelen.",
                    });
                }
            }
        }
    ...
    async analyze(batchId) {
        ...
        for (const staged of batch.rows) {
            const row = staged.parsedPayload;
        ...
        const summaries = await Promise.all(brands.map((brand) => this.brandSummary(brand, mappings)));
        const rows = [...groups.entries()]
            .map(([normalized, group]) => this.classify(batch, normalized, group, summaries, mappings))
            .sort((a, b) => a.sourceValue.localeCompare(b.sourceValue, "hu") ||
        ...
        const complete = rows.filter((row) => completed.has(row.classification)).length;
        return {
            rows,
            summary: {
                total: rows.length,
    ...
    classify(batch, normalized, group, brands, mappings) {
        const canonical = brands.filter((brand) => brand.normalizedName === normalized);
        const alias = brands.filter((brand) => brand.aliases.some((item) => item.normalizedAlias === normalized));
        const external = mappings.find((item) => item.externalKey && normalizeBrandName(item.externalKey) === normalized);
        const externalBrand = external
            ? brands.find((brand) => brand.id === external.entityId)
            : undefined;
        const matches = [
            ...new Map([...canonical, ...alias, ...(externalBrand ? [externalBrand] : [])].map((brand) => [brand.id, brand])).values(),
        ];
        const sourceTokens = new Set(normalized.split(" ").filter((token) => token.length > 2));
        const plausible = brands.filter((brand) => brand.isActive &&
        ...
        if (matches.length > 1) {
            ...
            matched = undefined;
            reasoning.push("Több aktív Brand osztozik jelentős névelemen; kézi választás szükséges.");
        }
        else {
        ...
        const updated = Math.max(batch.updatedAt.getTime(), ...matches.map((brand) => new Date(brand.updatedAt).getTime()), external?.updatedAt.getTime() ?? 0);
        return {
            id: rowId(normalized),
            sourceValue,
            ...
            matchedAlias: matched?.aliases.find((item) => item.normalizedAlias === normalized)?.alias,
    ...
    async brandSummary(brand, mappings) {
        return {
            id: brand.id,
            name: brand.name,
            ...
            aliases: brand.aliases.map((alias) => ({
                ...alias,
    ...
    async currentRow(batchId, id, expected) {
        const row = (await this.analyze(batchId)).rows.find((item) => item.id === id);
        if (!row)
            throw new NotFoundException("Az asszisztens sor nem található.");
        if (row.updatedAt !== expected)
            throw new ConflictException("Az asszisztens állapota időközben megváltozott.");
        return row;
    }
    async assertMutableBatch(batchId) {
        const batch = await prisma.catalogImportBatch.findUnique({
            where: { id: batchId },
        ...
        if (!["VALIDATED", "APPROVED"].includes(batch.status))
            throw new ConflictException("Ebben a batch státuszban a master-data nem módosítható.");
    }
    async reclassified(batchId, id) {
        return (await this.analyze(batchId)).rows.find((row) => row.id === id);
    }
    async assertIdentityFree(tx, normalized, allowedBrandId) {
        ...
        if ((brand && brand.id !== allowedBrandId) ||
    ...
    map(error) {
        if (error instanceof Error &&
            ...
            throw new ConflictException("A forrásérték vagy külső mapping már másik Brand tulajdona.");
        if (error instanceof Prisma.PrismaClientKnownRequestError &&
            error.code === "P2002")
            throw new ConflictException("A márkanév, alias vagy mapping időközben foglalttá vált.");
        throw error;
    }
};
BrandImportAssistantService = __decorate([
    Injectable()
], BrandImportAssistantService);
export { BrandImportAssistantService };
//# sourceMappingURL=brand-import-assistant.service.js.map