...
let BrandsRepository = class BrandsRepository extends Repository {
    constructor() {
        super(prisma);
    }
    async list(query) {
        ...
        const items = await Promise.all(brands.map((brand) => this.toDetail(brand)));
        return {
            items,
            pagination: {
                page: query.page,
    ...
    create(input, actorId) {
        ...
        return prisma.$transaction(async (tx) => {
            if (await tx.brandAlias.findFirst({
                where: { normalizedAlias: normalizedName },
            }))
                throw new Error("IDENTITY_CONFLICT");
            const aliases = input.aliases.filter((alias) => normalize(alias.alias) !== normalizedName);
    ...
    setArchived(id, archived, actorId) {
        return prisma.$transaction(async (tx) => {
            ...
            await this.event(tx, archived ? "brand.archived" : "brand.restored", id, actorId, { name: brand.name });
            return this.toDetail(brand, tx);
        }, { isolationLevel: Prisma.TransactionIsolationLevel.Serializable });
    }
    addAlias(brandId, input, actorId) {
        return prisma.$transaction(async (tx) => {
            const brand = await tx.brand.findUniqueOrThrow({
                where: { id: brandId },
            });
    ...
    async updateAlias(brandId, aliasId, input, actorId) {
        return prisma.$transaction(async (tx) => {
    ...
    removeAlias(brandId, aliasId, actorId) {
        return prisma.$transaction(async (tx) => {
            const alias = await tx.brandAlias.findFirstOrThrow({
                where: { id: aliasId, brandId },
            });
    ...
    async toDetail(brand, client = prisma) {
        ...
        return {
            ...
            externalMappings: mappings.map((mapping) => ({
                id: mapping.id,
