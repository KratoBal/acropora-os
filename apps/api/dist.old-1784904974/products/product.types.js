...
function channelSummary(listing) {
    return {
...
export function toProductListItem(product) {
    ...
    const unasListing = product.channelListings.find((listing) => listing.channel === "UNAS");
    ...
    return {
        ...
        stockOnHand: stockItems.length > 0
            ? stockItems
...
export function toProductDetail(product, externalId = null) {
    ...
    return {
        ...
        variants: product.variants.map((variant) => ({
            ...
            extension: variant.extension
                ? {
                    ...
                    lastPurchaseNetPrice: variant.extension.lastPurchaseNetPrice?.toString() ?? null,
                    lastPurchaseVatRate: variant.extension.lastPurchaseVatRate?.toString() ?? null,
        ...
        unasMirror: product.mirrorSource === "UNAS"
            ? {
                source: "UNAS",
                ...
                reportedStockSyncedAt: snapshot?.reportedStockSyncedAt?.toISOString() ?? null,
            }
            : null,
    };
}
