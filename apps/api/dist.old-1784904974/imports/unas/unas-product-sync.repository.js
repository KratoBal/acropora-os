...
const eventId = (...parts) => createHash("sha256").update(parts.join("|")).digest("hex");
const categorySlug = (category) => `unas-${category.externalId}-${category.name
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-|-$/g, "") || "category"}`;
const absoluteHttpUrl = (candidate) => {
    if (!candidate)
        return null;
    try {
...
const snapshotData = (product, syncedAt) => ({
...
let UnasProductSyncRepository = class UnasProductSyncRepository extends Repository {
    constructor() {
        super(prisma);
    }
    async getCursor() {
    ...
    async heartbeat(runId) {
        ...
        if (!result.count)
            throw new Error("UNAS_PRODUCT_SYNC_RUN_NOT_ACTIVE");
    }
    async getRun(runId) {
        ...
        if (!run)
            throw new NotFoundException("UNAS_PRODUCT_SYNC_RUN_NOT_FOUND");
        return run;
    }
    listRuns(limit) {
    ...
    async apply(runId, diffs, windowStart, windowEnd, categories, deletedExternalIds) {
        return prisma.$transaction(async (transaction) => {
            ...
            const existingCategoryReferences = await transaction.externalReference.findMany({
            ...
            for (const category of categories) {
                ...
                if (category.parentExternalId && !parentId)
                    throw new Error(`UNAS_CATEGORY_PARENT_NOT_FOUND:child=${category.externalId}:parent=${category.parentExternalId}`);
                await transaction.category.update({
                    where: { id },
                    data: { parentId },
                });
            ...
            for (const diff of diffs) {
                ...
                const product = diff.action === "CREATE"
                ...
                const referenceByEntity = await transaction.externalReference.findUnique({
                    where: {
                ...
                const normalizedImages = diff.product.images.map((image, index) => {
                    ...
                    if (!url)
                        throw new Error("UNAS_IMAGE_URL_NOT_ABSOLUTE");
                    return {
                        productId: product.id,
                        url,
...
UnasProductSyncRepository = __decorate([
    Injectable(),
    __metadata("design:paramtypes", [])
], UnasProductSyncRepository);
export { UnasProductSyncRepository };
//# sourceMappingURL=unas-product-sync.repository.js.map