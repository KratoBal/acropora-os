...
var UnasProductSyncScheduler_1;
...
export function unasSyncScheduleConfig(environment = process.env) {
    const enabled = environment.UNAS_PRODUCT_SYNC_ENABLED === "true";
    if (!enabled)
        return { enabled: false, intervalMs: 0, startupDelayMs: 0 };
    const intervalMinutes = boundedInteger(environment.UNAS_PRODUCT_SYNC_INTERVAL_MINUTES, 15, 5, 1440, "INVALID_UNAS_PRODUCT_SYNC_INTERVAL_MINUTES");
    const startupDelaySeconds = boundedInteger(environment.UNAS_PRODUCT_SYNC_STARTUP_DELAY_SECONDS, 30, 0, 3600, "INVALID_UNAS_PRODUCT_SYNC_STARTUP_DELAY_SECONDS");
    return {
        enabled,
...
let UnasProductSyncScheduler = UnasProductSyncScheduler_1 = class UnasProductSyncScheduler {
    auth;
    sync;
    logger = new Logger(UnasProductSyncScheduler_1.name);
    timer = null;
    stopped = false;
    ...
    onModuleInit() {
        ...
        if (!config.enabled)
            return;
        this.logger.log(`UNAS product sync scheduler enabled (${config.intervalMs / 60_000} min)`);
...
UnasProductSyncScheduler = UnasProductSyncScheduler_1 = __decorate([
    Injectable(),
    __metadata("design:paramtypes", [UnasAuthService,
        UnasProductSyncService])
], UnasProductSyncScheduler);
export { UnasProductSyncScheduler };
//# sourceMappingURL=unas-product-sync.scheduler.js.map