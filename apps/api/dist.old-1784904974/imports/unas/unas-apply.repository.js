...
const slug = (value) => normalizeBrandText(value).replace(/ /g, "-") || "category";
...
const splitReferences = (value) => (value ?? "")
    .split(/[|;,]/)
    .map((item) => item.trim())
    .filter(Boolean);
let UnasApplyRepository = class UnasApplyRepository extends Repository {
    constructor() {
        super(prisma);
    }
    getBatch(batchId) {
    ...
    async approve(batchId, actorId, decisions) {
        return prisma.$transaction(async (transaction) => {
            ...
            const byRow = new Map(decisions.map((decision) => [decision.sourceRowNumber, decision]));
            const now = new Date();
                ...
                for (const review of batch.brandResolutionReviews) {
                    ...
                    await transaction.brandResolutionReview.update({
                        ...
                        data: {
                            ...
                            resolvedBrandKey: decision.decision === "ACCEPT" ? decision.brandKey : null,
                            reviewedBy: actorId,
                            reviewedAt: now,
                        },
                    });
                }
            else if (batch.brandResolutionReviews.some((review) => review.status !== "ACCEPTED" && review.status !== "NO_BRAND"))
    ...
    async apply(batchId, actorId, expectedAnalysisVersion) {
        ...
        return prisma.$transaction(async (transaction) => {
            ...
            if (batch.status === "APPLIED" && batch.applyReport)
                return parsed(batch.applyReport);
            if (batch.status !== "APPROVED")
            ...
            const categoryIds = await this.upsertCategories(transaction, categories, counts);
            const brandIds = await this.brandIdsByDictionaryKey(transaction);
            const resolutionByRow = new Map((report.brandResolution?.products ?? []).map((resolution) => [
                resolution.sourceRowNumber,
                resolution,
            ]));
            const reviewByRow = new Map(batch.brandResolutionReviews.map((review) => [
            ...
            for (const row of products) {
                ...
                const existingReference = await transaction.externalReference.findUnique({
                    where: {
                ...
                const existingId = existingReference?.entityId ?? existingVariant?.productId;
                ...
                if (brandKey && !brandId)
                    counts.unresolvedBrandAssociations += 1;
                ...
                await this.syncProductDetails(transaction, product.id, row, categoryIds, counts);
            ...
            await this.syncRelations(transaction, relations, productIdsBySku, counts);
            await transaction.domainEvent.create({
                data: {
    ...
    async upsertCategories(transaction, categories, counts) {
        const ids = new Map();
    ...
    async brandIdsByDictionaryKey(transaction) {
        ...
        for (const brand of brands) {
            ...
            const entry = BRAND_DICTIONARY.find((candidate) => candidate.aliases.some((alias) => normalizedNames.includes(normalizeBrandText(alias))));
            if (entry)
                result.set(entry.key, brand.id);
        }
        return result;
    }
    async syncProductDetails(transaction, productId, row, categoryIds, counts) {
        ...
        const existingImageUrls = new Set((await transaction.productImage.findMany({
            where: { productId },
            select: { url: true },
        })).map((image) => image.url));
        const newImages = (row.imageUrls ?? []).filter((url) => !existingImageUrls.has(url));
        if (newImages.length)
    ...
    async syncRelations(transaction, products, productIdsBySku, counts) {
        ...
        for (const { productId, row } of products) {
            ...
            for (const [field, relationType] of relationFields) {
                ...
                for (const [sortOrder, sku] of references.entries()) {
                    const targetProductId = productIdsBySku.get(sku) ??
...
UnasApplyRepository = __decorate([
    Injectable(),
    __metadata("design:paramtypes", [])
], UnasApplyRepository);
export { UnasApplyRepository };
//# sourceMappingURL=unas-apply.repository.js.map