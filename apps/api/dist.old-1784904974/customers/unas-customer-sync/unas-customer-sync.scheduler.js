...
var UnasCustomerSyncScheduler_1;
...
export function unasCustomerSyncScheduleConfig(environment = process.env) {
    const enabled = environment.UNAS_CUSTOMER_SYNC_ENABLED === "true";
    if (!enabled)
        return { enabled: false, intervalMs: 0, startupDelayMs: 0 };
    const intervalMinutes = boundedInteger(environment.UNAS_CUSTOMER_SYNC_INTERVAL_MINUTES, 15, 1, 1440, "INVALID_UNAS_CUSTOMER_SYNC_INTERVAL_MINUTES");
    const startupDelaySeconds = boundedInteger(environment.UNAS_CUSTOMER_SYNC_STARTUP_DELAY_SECONDS, 30, 0, 3600, "INVALID_UNAS_CUSTOMER_SYNC_STARTUP_DELAY_SECONDS");
    return {
        enabled,
...
let UnasCustomerSyncScheduler = UnasCustomerSyncScheduler_1 = class UnasCustomerSyncScheduler {
    auth;
    sync;
    logger = new Logger(UnasCustomerSyncScheduler_1.name);
    timer = null;
    stopped = false;
    ...
    onModuleInit() {
        ...
        if (!config.enabled)
            return;
        this.logger.log(`UNAS customer sync scheduler enabled (${config.intervalMs / 60_000} min)`);
...
UnasCustomerSyncScheduler = UnasCustomerSyncScheduler_1 = __decorate([
    Injectable(),
    __metadata("design:paramtypes", [UnasAuthService,
        UnasCustomerSyncService])
], UnasCustomerSyncScheduler);
export { UnasCustomerSyncScheduler };
//# sourceMappingURL=unas-customer-sync.scheduler.js.map