...
import { UnasOrderStockAuditRepository, type UnasOrderStockAuditDatabase } from "./unas-order-stock-audit.repository.js";
...
describe("UnasOrderStockAuditService.auditPage", () => {
  it("decorates a row with DUPLICATE_UNAS_KEY when its UNAS key is shared by another order", async () => {
    ...
    const service = new UnasOrderStockAuditService(new UnasOrderStockAuditRepository(db));
    const page = await service.auditPage({ page: 1, pageSize: 10 });
    assert.ok(page.items.every((row) => row.riskFlags.includes("DUPLICATE_UNAS_KEY")));
  });
  ...
  it("paginates across many orders", async () => {
    ...
    for (let index = 0; index < 3; index += 1) {
      db.orders.push({ id: `order-${index}`, orderNumber: `UNAS-${index}`, status: "CONFIRMED", lines: [] });
    }
    const service = new UnasOrderStockAuditService(new UnasOrderStockAuditRepository(db));
...
describe("UnasOrderStockAuditService.findAnomalies", () => {
  it("never mutates any table - both underlying repository queries are pure reads", async () => {
    ...
    const service = new UnasOrderStockAuditService(new UnasOrderStockAuditRepository(db));
    const before = JSON.stringify(db.orders) + JSON.stringify(db.references) + JSON.stringify(db.movements);
    await service.findAnomalies();
    const after = JSON.stringify(db.orders) + JSON.stringify(db.references) + JSON.stringify(db.movements);
